document.addEventListener('DOMContentLoaded', () => {
    ...
    // Event listener to download the list as PDF
    downloadPdfButton.addEventListener('click', () => {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();
        const currentListName = localStorage.getItem('currentListName') || "Current List";
        doc.text(currentListName, 10, 10);
        const lists = JSON.parse(localStorage.getItem('lists')) || {};
        const currentList = lists[currentListName] ? lists[currentListName].items : [];
        let yPosition = 20;
        if (currentList.length === 0) {
            doc.text('No items found', 10, yPosition);
        } else {
            currentList.forEach((item, index) => {
                doc.text(`${index + 1}. ${item.name} (${item.quantity} ${item.unit}) - ${item.category}`, 10, yPosition);
                yPosition += 10;
            });
        }
